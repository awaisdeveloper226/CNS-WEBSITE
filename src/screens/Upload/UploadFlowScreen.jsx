...
const Icons = {
  ArrowLeft: ({ size, color }) => (
    <Icon size={size} color={color} d="M19 12H5M12 19l-7-7 7-7" />
  ),
  Package: ({ size, color }) => (
    <Icon
      size={size}
  ...
  UtensilsCrossed: ({ size, color }) => (
    <Icon
      size={size}
      color={color}
      d={[
        "M3 2l7.5 7.5",
        "M7.5 9.5L2 15l5 5 6-6",
        "M21 2l-7.5 7.5",
        "M16.5 9.5L22 15l-5 5-6-6",
        "M12 12l2-2",
      ]}
    />
  ),
  ...
  Send: ({ size, color }) => (
    <Icon
      size={size}
      color={color}
      d={["M22 2L11 13", "M22 2L15 22 11 13 2 9l20-7z"]}
    />
  ),
  Trash2: ({ size, color }) => (
    <Icon
  ...
  Navigation: ({ size, color }) => (
    <Icon size={size} color={color} d="M3 11l19-9-9 19-2-8-8-2z" />
  ),
  ...
  ParkingSquare: ({ size, color }) => (
    <Icon
      size={size}
      color={color}
      d={["M3 3h18v18H3z", "M9 17V7h4a3 3 0 010 6H9"]}
    />
  ),
  Compass: ({ size, color }) => (
    <Icon
...
const mapCategoryToBackend = (category, deliveryType) => {
  ...
  if (deliveryType === "Courier/Parcel Delivery")
    return "Courier/Parcel Delivery Instructions";
          ...
          <div className="map-modal-header-left">
            ...
            <div>
              ...
              <div className="map-modal-sub">
                Tap any location to autofill details
              </div>
            </div>
          </div>
            ...
            <>
              <div className="map-location-row">
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div className="map-location-addr">
                    {pickedResult.address}
                  </div>
                </div>
              </div>
              <button
...
const WhatsAppVoiceRecorder = ({ onAudioReady, disabled = false }) => {
  ...
  const onPointerMove = (e) => {
    ...
    const currentX =
      e.clientX || (e.touches?.[0]?.clientX ?? startXRef.current);
    const currentY =
      e.clientY || (e.touches?.[0]?.clientY ?? startYRef.current);
      ...
      <div className="wa-locked">
        <div className="wa-locked-top">
          <div className="wa-locked-timer-row">
            ...
            <span className="wa-locked-timer">
  ...
  useEffect(() => {
    ...
    fetch(`${API_ENDPOINTS.PLACES_SEARCH}?q=${encodeURIComponent(query)}`, {
      ...
      .catch((err) => {
        ...
        setSearchError("Could not fetch suggestions. Try again.");
      })
  ...
  const handleSubmit = async () => {
    ...
    try {
      const syntheticPlaceId =
        pickedFromMap && !selectedPlaceId && pickedCoords
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
      ...
      <div className="upload-content">
        ...
        <div className="form-subtitle">
          Search by name or address, or pin a spot on the map.
        </div>
              ...
              {suggestions.map((place, idx) => (
                ...
                >
                  <div className="suggestion-icon">
                    <Icons.MapPin size={15} color="#2563eb" />
                  </div>
            ...
            debouncedSearch.trim().length > 1 && (
              <div className="dropdown">
                <div className="no-results-row">
                  No places found. Try a different search or use the map below.
                </div>
              </div>
            )}
        </div>
        ...
        >
          <div className="map-picker-icon">
            <Icons.Map size={20} color="#2563eb" />
          </div>
        ...
        {hasSelection && (
          <div className={`confirmed-card${pickedFromMap ? " map" : ""}`}>
            <div className="confirmed-icon">
              {pickedFromMap ? (
                <Icons.Navigation size={20} color="#059669" />
              ) : (
                <Icons.Building2 size={20} color="#2563eb" />
              )}
            </div>
...
export default function UploadFlowScreen({
  ...
  const [step, setStep] = useState(
    initialBusiness ? "details" : "create-business",
  );
  ...
  const handleSubmitContribution = async () => {
    ...
    try {
      ...
      if (isUnregisteredBusiness(selectedBusiness)) {
        const r = await fetch(API_ENDPOINTS.BUSINESSES, {
          ...
          body: JSON.stringify({
            ...
            source: ["manual", "foursquare", "nominatim"].includes(
        ...
        setSelectedBusiness((prev) =>
          prev ? { ...prev, _id: d._id, id: d.id } : prev,
        );
      } else {
      ...
      if (onComplete)
        onComplete({ ...selectedBusiness, _id: businessId, id: businessId });
    } catch (err) {
    ...
    return (
      <div className="upload-shell">
        <div className="upload-container">
          <div className="upload-header">
            <button
              className="upload-header-back"
              onClick={() =>
                initialBusiness ? onBack?.() : setStep("create-business")
              }
            >
              <Icons.ArrowLeft size={24} color="#1f2937" />
            </button>
          ...
          <div className="upload-content">
            <div className="selected-business-name">
                ...
                (dt) => (
                  ...
                  >
                    {dt === "Courier/Parcel Delivery" && (
                      <Icons.Package
                        size={18}
                        color={deliveryType === dt ? "#fff" : "#6b7280"}
                      />
                    )}
                    {dt === "Food Delivery" && (
                ...
                {audioUrl ? (
                  <div className="audio-preview">
                    <div className="audio-preview-header">
                      <span className="audio-preview-title">
                        🎵 Audio Instruction
                      </span>
                      <button
                  ...
                  <>
                    ...
                    {uploadingFileAudio ? (
                      ...
                      <button
                        className="audio-upload-file-btn"
                        onClick={handleAudioFromStorage}
                      >
            ...
            >
              <div
                className={`owner-checkbox${isBusinessOwner ? " active" : ""}`}
              >
                {isBusinessOwner && (
                  <Icons.CheckCircle size={16} color="#fff" />
                )}
              </div>
              <div className="owner-text">
                <div className="owner-label-row">
                  <Icons.ShieldCheck
                    size={14}
                    color={isBusinessOwner ? "#059669" : "#6b7280"}
                  />
                  <span className="owner-label">I am the business owner</span>
                </div>
                <div className="owner-sublabel">
                  Your instruction will show a "Business Owner" badge so
                  couriers know it's official.
                </div>
              </div>
            </div>
        ...
        <div className="upload-container">
          ...
          <div className="upload-content">
            <div className="preview-label">
              Your contribution will appear like this:
            </div>
            ...
            <div className="contribution-card">
              <div className="contribution-header">
                <div className="contribution-user">
                  ...
                  <div style={{ marginLeft: 10 }}>
                    ...
                    <div className="contribution-user-level">
                      Level {user.level}
                    </div>
                  </div>
                </div>
                <div className="delivery-type-badges">
                  {(deliveryType === "Courier/Parcel Delivery" ||
                    deliveryType === "Both") && (
              ...
              {isBusinessOwner && (
                <div className="owner-preview-badge">
                  ...
                  <span className="owner-preview-badge-text">
              ...
              {instructionMode === "record" && audioUrl && (
                <div className="audio-preview-in-card">
                  <span className="audio-preview-in-card-text">
                    🎵 Audio Instruction
                  </span>
                  {audioDuration && (
